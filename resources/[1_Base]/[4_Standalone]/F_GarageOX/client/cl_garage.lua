        ...
        Citizen.CreateThread(function()
            while open do 
                ...
                for i = 1, #Garage.vehiclelist, 1 do
                    local vehicleHash = Garage.vehiclelist[i].vehicle.model
                    local modelevehiclespawn = Garage.vehiclelist[i].vehicle
                    local nomvehiclemodele = GetDisplayNameFromVehicleModel(vehicleHash)
                    local plaque = Garage.vehiclelist[i].plate
        
        
                    RageUI.Button(nomvehiclemodele.." | "..plaque, "Pour sortir votre véhicule", {RightLabel = "→→→"}, true, {
                        onSelected = function()  
